        ...
        <div class="flex-1 mb-4 md:mb-0">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
            ...
            <div class="bg-[#F49131] bg-opacity-10 p-3 md:p-4 rounded-lg">
              ...
              <div class="w-full bg-gray-200 h-2 md:h-3 rounded-full mt-2 overflow-hidden">
                <div class="bg-[#F49131] h-full rounded-full transition-all duration-700"
                     :style="{width: `${Math.min(totalElevation / 29032 * 100, 100)}%`}"></div>
              </div>
              <p class="text-xs md:text-sm text-gray-600 mt-1 md:mt-2">{{ (totalElevation / 29032 * 100).toFixed(1) }}% of Everest</p>
            </div>
            ...
            <div class="bg-[#FFC72C] bg-opacity-10 p-3 md:p-4 rounded-lg">
              ...
              <p class="text-xs text-gray-500">Mostly as the sun comes up! ☀️</p>
            </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full">
          ...
          <tr class="bg-gradient-to-r from-[#005eb8] to-[#22ABE2] text-white">
            <th class="py-2 px-2 md:py-3 md:px-4 text-left rounded-tl-lg">Date</th>
            ...
            <th class="py-2 px-2 md:py-3 md:px-4 text-left rounded-tr-lg">Notes</th>
          </tr>
          </thead>
          <tbody>
          <tr v-for="(ride, index) in sortedRides" :key="index"
            ...
            <td class="py-2 px-2 md:py-3 md:px-4 text-right whitespace-nowrap">{{ ride.distance.toFixed(2) }} mi</td>
            <td class="py-2 px-2 md:py-3 md:px-4 text-right whitespace-nowrap">{{ ride.elevation.toLocaleString() }} ft</td>
            <td class="py-2 px-2 md:py-3 md:px-4 text-right whitespace-nowrap">{{ formatTime(ride.duration) }}</td>
            <td class="py-2 px-2 md:py-3 md:px-4">
              <span class="text-gray-600 italic text-sm md:text-base">{{ ride.note || '-' }}</span>
            </td>
          </tr>
          </tbody>
          <tfoot>
          <tr class="bg-gray-100 font-semibold">
            <td class="py-2 px-2 md:py-3 md:px-4 rounded-bl-lg" colspan="2">Total</td>
            <td class="py-2 px-2 md:py-3 md:px-4 text-right">{{ totalDistance.toFixed(2) }} mi</td>
            <td class="py-2 px-2 md:py-3 md:px-4 text-right">{{ totalElevation.toLocaleString() }} ft</td>
            <td class="py-2 px-2 md:py-3 md:px-4 text-right">{{ formatTime(totalDuration) }}</td>
...
// Computed properties
const totalDistance = computed(() => rides.value.reduce((sum, ride) => sum + ride.distance, 0))
const totalElevation = computed(() => rides.value.reduce((sum, ride) => sum + ride.elevation, 0))
const totalDuration = computed(() => rides.value.reduce((sum, ride) => sum + ride.duration, 0))
const averageElevationPerRide = computed(() => totalElevation.value / rides.value.length)
